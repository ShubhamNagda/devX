        ...
        <div className="lg:relative z-10 absolute bottom-3 w-11/12 h-2/12 lg:h-11/12 flex lg:flex-col items-center justify-around rounded-xl bg-[#d4cfcf15] backdrop-blur-2xl backdrop-saturate-150 border border-white/30 shadow-2xl shadow-blue-950/20 ring-1 ring-white/10">
            <div className="lg:w-9/12 flex flex-row lg:flex-col items-center justify-around">
                <ul className="w-full flex lg:items-start items-center justify-evenly flex-row lg:flex-col text-xl lg:text-2xl gap-2 lg:gap-5 text-[#ffffff]">
                    ...
                    <li onClick={() => setNavigation("Home")} className={`flex font-bold gap-2 lg:gap-4 cursor-pointer hover:text-[#d429e7e7] hover:scale-95 ${ navigation === "Home" ? "text-[#ffffff]" : "text-[#c5bdbddc]"}`}><House size={30} strokeWidth={2.25} /> Home</li>
                    <li onClick={() => setNavigation("Search")} className={`flex font-bold gap-2 lg:gap-4 cursor-pointer hover:text-[#d429e7e7] hover:scale-95 ${ navigation === "Search" ? "text-[#ffffff]" : "text-[#c5bdbddc]"}`}><Search size={30} strokeWidth={2.25} /> Search</li>
                    <li onClick={() => setNavigation("MitraAI")} className={`flex font-bold gap-2 lg:gap-4 cursor-pointer hover:text-[#d429e7e7] hover:scale-95 ${ navigation === "MitraAI" ? "text-[#ffffff]" : "text-[#c5bdbddc]"}`}> <Astroid size={30} strokeWidth={2.25} /> Mitra AI</li>
                    <li onClick={() => setNavigation("EditInfo")} className={`flex font-bold gap-2 lg:gap-4 cursor-pointer hover:text-[#d429e7e7] hover:scale-95 ${ navigation === "EditInfo" ? "text-[#ffffff]" : "text-[#c5bdbddc]"}`}><UserRoundCog size={30} strokeWidth={2.25} /> Edit Info</li>
                </ul>
            </div>
            <div className="lg:w-9/12 flex flex-col items-center ">
                <ul className="w-full flex justify-center items-center lg:items-start flex-row lg:flex-col text-2xl gap-5 text-[#ffffff]">
